...
def model(features, labels, mode, params):
    ...
    with tf.variable_scope('encoder_scope', reuse=tf.AUTO_REUSE):
        ...
        if params['multilayer'] == True:
            encoder_cell_list = [make_lstm_cell(mode, params['ffn_hidden_size'], i) for i in range(params['layer_size'])]
            rnn_cell = tf.contrib.rnn.MultiRNNCell(encoder_cell_list)
        else:
            rnn_cell = make_lstm_cell(mode, params['ffn_hidden_size'], "")
    ...
    with tf.variable_scope('decoder_scope', reuse=tf.AUTO_REUSE):
        if params['multilayer'] == True:
            decoder_cell_list = [make_lstm_cell(mode, params['ffn_hidden_size'], i) for i in range(params['layer_size'])]
